...
public class UpdateReviewCommandHandler(IUnitOfWork _unitOfWork, IMapper _mapper, IPublishEndpoint _publishEndpoint)
    : IUpdateCommandHandler<UpdateReviewCommand, ReviewResponseDto>
{
    public async Task<Shared.Wrappers.Response<ReviewResponseDto>> Handle(UpdateReviewCommand request, CancellationToken cancellationToken)
    {
